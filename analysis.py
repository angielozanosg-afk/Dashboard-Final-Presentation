...
def run_clustering(df, k=4):
    ...
    prof = prof.sort_values("WTP_SGD", ascending=False)
    names = ["Priority Converter" if r.Priority_Rate >= 0.55
             else "Emerging Buyer" if r.Priority_Rate >= 0.30
             else "Pre-seed Explorer" for r in prof.itertuples()]
    prof.insert(0, "Profile", names)
