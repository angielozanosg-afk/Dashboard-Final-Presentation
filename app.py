...
tabs = st.tabs([
    ...
    "3 - Who to Target", "4 - The Money", "5 - The Playbook", "6 - Methodology"])
...
with tabs[0]:
    ...
    tag("SUMMARY")
    callout("story", "",
            "<p>The entire analysis in one screen: what the data tells "
            "BridgeCompliance to do to enter the market. The strategic question is "
            "not <i>which city</i> but <i>which customer</i> - and the evidence for "
            "every claim below lives in the tabs that follow.</p>")

    cards = [
        ("Which market to enter", "Enter Dubai first - but the city is not the lever",
         "The two markets are statistically tied on demand. Dubai skews to later-stage "
         "startups that pay more, so it is the better first beachhead - yet the real "
         "advantage is the customer profile, not the location."),
        ("Who to target", "The 'Priority Converter' profile",
         "Seed to Series A-B, Fintech or Crypto, high urgency, 2-4 active regulatory "
         "pains. This group both wants the service and can pay for it."),
    ...
    st.markdown("#### The decision cascade")
    st.markdown(
        "<div class='casc'>"
        "<div class='step'><div class='n'>1. MARKET</div><div class='t'>Dubai first</div>"
        "<div class='d'>Later-stage skew, higher paying base</div></div>"
        "<div class='step'><div class='n'>2. SEGMENT</div><div class='t'>Priority Converters</div>"
        "<div class='d'>Seed-Series A-B, Fintech/Crypto, urgent</div></div>"
        "<div class='step'><div class='n'>3. OFFER</div><div class='t'>Compliance bundle</div>"
        "<div class='d'>AML/CFT + Licensing + Data Protection</div></div>"
        "<div class='step'><div class='n'>4. PRICE</div><div class='t'>Three tiers</div>"
        "<div class='d'>By stage; urgency triggers the upsell</div></div>"
        "<div class='step'><div class='n'>5. CHANNEL</div><div class='t'>Ranked outreach</div>"
        "<div class='d'>Call highest expected-value leads first</div></div>"
...
with tabs[5]:
    ...
    show = prof.rename(columns={"WTP_SGD": "WTP", "Priority_Rate": "Priority %",
                                "Pain_Count": "Pains", "Dubai_%": "Dubai %"})
    show["Priority %"] = (show["Priority %"] * 100).round(0)
    show["WTP"] = show["WTP"].apply(money)
    st.dataframe(show[["Segment", "Profile", "Urgency", "Likelihood", "WTP", "Stage",
    ...
    with c1:
        ...
        for i, (_, rr) in enumerate(nm.iterrows()):
            fig.add_trace(go.Scatterpolar(r=[rr[a] for a in axes] + [rr[axes[0]]],
                ...
                fill="toself", name=f"S{int(rr['Segment'])} {rr['Profile']}", line_color=pal[i % 4]))
        fig.update_layout(polar=dict(radialaxis=dict(visible=True, range=[0, 1])))
        chart(fig, "Segment fingerprints", 360, True)
    with c2:
        fig = px.pie(prof, values="N", names="Profile", hole=0.5,
                     color_discrete_sequence=[BLUE, AQUA, YELLOW, VIOLET])
        chart(fig, "Segment sizes", 360, True)
...
# =========================================================================== #
# TAB 6 - METHODOLOGY (TECHNICAL APPENDIX)                                     #
# =========================================================================== #
with tabs[6]:
